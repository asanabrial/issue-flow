...
| S15 | Abandoned work (`SKILL.md:743-840`) | `references/safety.md` | Keep horizon, heartbeat, reclaim, and retention failure modes together. | no | no |
...
| ID | Current source | Final owner/path | Intentional-consolidation rationale | Owner copied | Old source retired |
|---|---|---|---|---|---|
| I01 | Mechanical steps omitted despite correct prose (`SKILL.md:154-162`) | `references/safety.md` | Preserve why reversible transport operations are executable. | no | no |
| I02 | Five-minute loop nearly selected second work (`SKILL.md:444-460`) | `references/safety.md` | Preserve why self-held work is checked before queues. | no | no |
| I03 | Issue #58 loser wrote into winner checkout (`SKILL.md:497-505`) | `references/safety.md` | Preserve the pre-filesystem claim-renewal incident. | no | no |
| I04 | Dev marker attached only at close (`SKILL.md:510-516`) | `references/safety.md` | Preserve why attribution is projected during transition. | no | no |
| I05 | Issue #58 shared worktree collision (`SKILL.md:656-664`) | `references/repository-delivery.md` | Preserve why worktree identity must prevent cross-run writes. | no | no |
| I06 | Five claims never transitioned (`SKILL.md:751-759`) | `references/safety.md` | Preserve the claim/transition partial-failure case. | no | no |
| I07 | Claim loser worked 48 minutes after adjudication (`SKILL.md:777-802`) | `references/safety.md` | Preserve why every heartbeat reads before writing. | no | no |
| I08 | Agent-team tasks left incomplete (`SKILL.md:761-766`) | `references/runtime-notes.md` | Preserve the external-session recovery limitation. | no | no |
| I09 | Wrong handoff state buried recoverable work (`SKILL.md:841-867`) | `references/safety.md` | Preserve why analysis, blocked, and review are distinct. | no | no |
| I10 | Long run repeated its documented `$?` mistake (`SKILL.md:887-900`) | `references/safety.md` | Preserve the evidence-based handoff threshold. | no | no |
| I11 | Issues #61/#62 never mirrored to the board (`SKILL.md:984-1005`) | `references/safety.md` | Preserve why transition verifies both state projections. | no | no |
| X01 | Codex rejects the slash skill sigil (`SKILL.md:27-34`) | `references/runtime-notes.md` | Consolidate runtime invocation differences. | no | no |
| X02 | Unknown labels fail at issue creation (`SKILL.md:121-125,967-971`) | `references/safety.md` | Preserve why create/ensure operations provision labels first. | no | no |
| X03 | Different priority scales cannot be globally ranked (`SKILL.md:130-141`) | `references/domain-composition.md` | Preserve partitioned selection semantics. | no | no |
| X04 | Loading the wrong routing-arrow side applies the wrong job rules (`SKILL.md:277-287`) | `references/domain-composition.md` | Preserve explicit analyst/dev rule-book selection. | no | no |
| X05 | Shell write access and teammate skill loading limits (`SKILL.md:352-364`) | `references/runtime-notes.md` | Consolidate runtime enforcement caveats. | no | no |
| X06 | Fresh worktrees omit ignored credentials and settings (`SKILL.md:666-672`) | `references/repository-delivery.md` | Preserve environment reuse requirements. | no | no |
| X07 | Rebase invalidates SHA evidence and shared refs (`SKILL.md:706-714`) | `references/repository-delivery.md` | Preserve the merge-by-default rationale. | no | no |
| X08 | Missing acceptance criteria invites a fabricated done bar (`SKILL.md:902-918`) | `references/safety.md` | Preserve the pre-build criteria gate. | no | no |
| X09 | Human-decision blockers become abandoned (`SKILL.md:937-958`) | `references/safety.md` | Preserve precise exit-condition and discharger requirements. | no | no |
| X10 | Parallel A/B died from machine contention (`SKILL.md:1053-1060`) | `references/runtime-notes.md` | Preserve resource-coordination and sequential-rerun guidance. | no | no |
...
| K13 | Analyst repository work is read-only and yields at most one issue. | `SKILL.md` | Keep the hard role boundary loaded. | no | no |
| K14 | Analyst drains analysis, then blocked, before new discovery. | `SKILL.md` | Preserve recovery-first queue order. | no | no |
| K15 | Analyst tool restrictions enforce the write boundary where possible. | `references/runtime-notes.md` | Consolidate runtime-specific enforcement. | no | no |
| K16 | Autonomous discovery deduplicates and files only the strongest evidenced finding. | `references/domain-composition.md` | Preserve the built-in domain bar. | no | no |
...
| K18 | Create carries every marker; incomplete analysis names a precise blocker. | `SKILL.md` | Preserve filing semantics. | no | no |
| K19 | A dev resumes self-held work before selecting another issue. | `SKILL.md` | Enforce one task per run. | no | no |
| K20 | Unassigned review work is selected before ready work. | `SKILL.md` | Finish published work first. | no | no |
| K21 | Claim ownership is timeline-adjudicated; earliest live claim wins. | `SKILL.md` | Preserve race resolution. | no | no |
| K22 | Claim is renewed before the first repository write and every irreversible boundary. | `SKILL.md` | Cap displaced-work cost. | no | no |
| K23 | Ready transitions once to in-progress with the dev marker and no duplicate claim. | `SKILL.md` | Preserve state and race evidence. | no | no |
| K24 | Issue criteria plus routed domain and repository rules define done. | `SKILL.md` | Prevent invented implementation scope. | no | no |
| K25 | Review target is published before transition to review. | `SKILL.md` | Bind review to shared bytes. | no | no |
| K26 | Review and CI name exact head/base SHAs; every push invalidates both. | `SKILL.md` | Prevent stale verdicts. | no | no |
| K27 | Built but undeliverable work stays in review and is never bypassed. | `SKILL.md` | Preserve honest delivery state. | no | no |
| K28 | Merge only the reviewed/green head and verify delivered topology. | `SKILL.md` | Preserve reviewed history. | no | no |
| K29 | Version tags are immutable, remote-verified, and release-aware. | `SKILL.md` | Keep publication a delivery gate. | no | no |
| K30 | Base is read-only; each issue uses one branch and isolated worktree. | `references/repository-delivery.md` | Consolidate repository isolation. | no | no |
| K31 | Worktree paths cannot be shared; resume only the branch's registered checkout. | `references/repository-delivery.md` | Preserve collision safety. | no | no |
| K32 | Fresh worktrees restore required ignored inputs and reuse the established environment. | `references/repository-delivery.md` | Prevent checkout-only failures. | no | no |
