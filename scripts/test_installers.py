...
def installer(repo: Path, name: str, upstream: str, legacy: bool = False) -> None:
    text = (git(ROOT, "show", f"origin/main:{name}") if legacy else
...
def repositories(root: Path) -> tuple[Path, Path, str, str, str]:
    ...
    installer(author, "install.sh", upstream, legacy=True)
    installer(author, "install.ps1", upstream, legacy=True)
    ...
    git(root, "clone", "--bare", author.as_posix(), upstream)
...
def clone_at(remote: Path, target: Path, revision: str) -> None:
    git(target.parent, "clone", remote.as_posix(), target.as_posix())
...
def runtime_copy(home: Path) -> Path:
    path = home / ".codex/skills/issue-flow"
...
for kind, executable in shells():
    with tempfile.TemporaryDirectory() as temp:
        root = Path(temp)
        author, remote, legacy, old, new = repositories(root)
        installed, home = root / "installed", root / "home"
        ...
        check(f"{kind} upgrade has a usable reflog rollback", git(installed, "rev-parse", "HEAD") == old
              and (installed / "removed.md").is_file() and not (installed / "added.md").exists())
        result = command(kind, executable, script, ["sync", "-DryRun"] if kind != "POSIX" else ["sync", "--dry-run"], home)
        check(f"{kind} dry-run leaves HEAD unchanged", result.returncode == 0 and git(installed, "rev-parse", "HEAD") == old)
        (installed / "references/runtime-notes.md").write_text("dirty\n", encoding="utf-8")
        result = command(kind, executable, script, ["sync"], home)
        check(f"{kind} refuses dirty tracked state", result.returncode != 0 and git(installed, "rev-parse", "HEAD") == old)
        ...
        result = command(kind, executable, script, args, home)
        check(f"{kind} retires --from before mutation", result.returncode != 0 and git(installed, "rev-parse", "HEAD") == old)
        git(installed, "switch", "-c", "feature")
        result = command(kind, executable, script, ["sync"], home)
        check(f"{kind} refuses non-main branches", result.returncode != 0)
        git(installed, "switch", "main")
        copy = runtime_copy(home)
        result = command(kind, executable, script, ["sync"], home)
        check(f"{kind} refuses stale runtime copies", result.returncode != 0 and git(installed, "rev-parse", "HEAD") == old)
        shutil.rmtree(copy)
        git(installed, "remote", "set-url", "origin", (root / "other.git").as_posix())
        result = command(kind, executable, script, ["sync"], home)
        check(f"{kind} binds upgrades to the canonical origin", result.returncode != 0 and git(installed, "rev-parse", "HEAD") == old)
        git(installed, "remote", "set-url", "origin", remote.as_posix())
        bootstrap_home = root / "bootstrap-home"
        bootstrap_installed = bootstrap_home / ".agents/skills/issue-flow"
        bootstrap_installed.parent.mkdir(parents=True); clone_at(remote, bootstrap_installed, legacy)
        external = root / ("install.ps1" if kind != "POSIX" else "install.sh")
        shutil.copy2(author / external.name, external)
        (bootstrap_installed / "references/runtime-notes.md").write_text("dirty\n", encoding="utf-8")
        result = command(kind, executable, external, ["sync"], bootstrap_home)
        check(f"{kind} bootstrap refuses dirty legacy installs", result.returncode != 0 and git(bootstrap_installed, "rev-parse", "HEAD") == legacy)
        git(bootstrap_installed, "reset", "--hard", legacy)
        dry_args = ["sync", "-DryRun"] if kind != "POSIX" else ["sync", "--dry-run"]
        result = command(kind, executable, external, dry_args, bootstrap_home)
        check(f"{kind} bootstrap honors dry-run before mutation", result.returncode == 0 and git(bootstrap_installed, "rev-parse", "HEAD") == legacy)
        result = command(kind, executable, external, args, bootstrap_home)
        check(f"{kind} bootstrap retires --from before mutation", result.returncode != 0 and git(bootstrap_installed, "rev-parse", "HEAD") == legacy)
        copy = runtime_copy(bootstrap_home)
        result = command(kind, executable, external, ["sync"], bootstrap_home)
        check(f"{kind} bootstrap checks runtime copies before mutation", result.returncode != 0 and git(bootstrap_installed, "rev-parse", "HEAD") == legacy)
        shutil.rmtree(copy)
        result = command(kind, executable, external, ["sync"], bootstrap_home)
        check(f"{kind} safely upgrades a real legacy installation", result.returncode == 0 and git(bootstrap_installed, "rev-parse", "HEAD") == new)
        git(installed, "reset", "--hard", old)
        ...
        result = command(kind, executable, script, ["sync"], home)
        check(f"{kind} preserves case-folded local path collisions", result.returncode != 0 and (installed / "Notes.Local").read_text() == "keep\n" and git(installed, "rev-parse", "HEAD") == old)
        ...
        result = command(kind, executable, script, ["sync"], home)
        check(f"{kind} rejects targets that track operator policy", result.returncode != 0 and (installed / "operator.local.md").read_bytes() == b"local-secret-policy\r\n" and git(installed, "rev-parse", "HEAD") == old)
        protected_home = root / "protected-home"; protected = protected_home / ".agents/skills/issue-flow"
        protected.parent.mkdir(parents=True); clone_at(remote, protected, legacy)
        result = command(kind, executable, external, ["sync"], protected_home)
        ...
        (author / "Operator.Local.md").unlink(); (author / ".gitignore").write_text("__pycache__/\n")
        git(author, "add", "-A"); git(author, "commit", "-m", "unignore policy"); git(author, "push", "origin", "main")
        result = command(kind, executable, script, ["sync"], home)
        check(f"{kind} rejects targets that expose operator policy", result.returncode != 0 and git(installed, "rev-parse", "HEAD") == old)
print()
