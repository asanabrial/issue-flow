...
| branch + worktree | `SCRIPT start-branch --issue <n> --branch <b> --base <base> --run-id <id>` | renews first, then serializes on a branch-scoped lock and reserves the whole local checkout **before any GitHub mutation**, so a lost reservation leaves no remote state. Resumes only a checkout whose durable ownership marker names this run; a branch-only template gains a run-scoped sibling in memory without rewriting `operator.local.md`. Ref existence is probed so absence and a failed read are different answers, a nonzero or timed-out `gh issue develop` is re-read rather than believed, and local head, published head and recorded base must agree before success is reported. It records the base COMMIT and the base TREE, so a later movement check compares against what the base contained rather than resolving it again. The local worktree registry is read NUL-delimited and fails closed: a failed, truncated or contradictory read is a failed read, never an empty registry |
| `publish_review` | `SCRIPT publish-review --issue <n> --branch <b> --base <base> --run-id <id> --pr-title <t> --pr-body-file <f> [--worktree <p>]` | pushes, **reuses** the single open PR or creates one, refuses on more than one, scans for closing keywords. A reused PR has its supplied title and body applied — otherwise a resumed delivery advertises the old description over the new head. The head and base it records are **read back** until the remote agrees with the push, never taken from the write path: `pr list` is served from an index that lags a push, and the head it returns immediately afterwards can be the previous one |
| `expected_target` | `SCRIPT expected-target --base <sha> [--worktree <p>] [--native-start <f>]` | read-only. Emits the COMPLETE delivery target — the exact base through `HEAD` plus the uncommitted worktree — as a path/mode/blob manifest with one digest. With `--native-start` it compares a reviewer's target against it and **fails closed**, naming the paths that would ship unreviewed. Uses `ls-tree`, `status` and `hash-object` without `-w`, so no object, ref, index or file is written |
| `base_movement` | `SCRIPT base-movement --base <branch> --recorded-base <sha> [--worktree <p>]` | read-only. Classifies later base movement as none, compatible, overlapping, conflicting or unknown, from exact refs, changed paths and `git merge-tree --write-tree` — which merges in memory, touching no index, worktree or ref. Unknown is never compatible, and semantic impact is reported as the caller's judgement rather than decided |
