...
| `claim` | `SCRIPT claim --issue <n> --run-id <id> --runtime <rt> --horizon <UTC>` | appends ownership before projecting assignee/label state, then accepts only the reducer's live winner. The future horizon must be `YYYY-MM-DDTHH:MM[:SS]Z`; an expired self-claim gets a fresh event, while a live metadata-less legacy claim fails closed rather than borrowing caller metadata; same-runtime loss cleanup preserves the winner's shared label |
| `reclaim` | `SCRIPT reclaim --issue <n> --run-id <id> --runtime <rt> [--horizon <UTC>] [--force --reason-file <f>]` | one trusted, validated event releases the named holder and establishes the new holder with a horizon before recoverable projections. Forced events created at or after `2026-07-27T19:00:00Z`, and edited earlier comments, are authoritative only when the comment carries the non-empty, escaped evidence read from `<f>`. An idempotent retry reuses persisted evidence and may omit `<f>` |
