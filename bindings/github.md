...
| `claim` | `SCRIPT claim --issue <n> --run-id <id> --runtime <rt> --horizon <UTC>` | an expired self-claim renews here; stale foreign contenders refuse before writing and route through `reclaim` |
| `reclaim` | `SCRIPT reclaim --issue <n> --run-id <id> --runtime <rt> [--horizon <UTC>] [--force --reason-file <f>]` | selects a stale foreign target even in a mixed self/foreign set. A verified heartbeat after acquisition extends the later of the declared horizon or one four-hour window, never permanent ownership |
...
| `unassign` | `SCRIPT unassign --issue <n> --runtime <rt> --run-id <id> [--held-by-other]` | requires matching persisted or unambiguous legacy runtime metadata. `--held-by-other` is valid only when the reducer confirms a live holder |
